<template>
  <div class="app-container">
    ...
    <div class="searchBar" @click="searchBlur">
      <a-popover v-model:open="visible" trigger="click" placement="bottom"  style="color: rgb(255, 255, 255);background: rgba(190, 182, 182, 0.1);backdropFilter: blur(10px)">
        <template #content>
        <div style="color: #fff;">
          <div class="icon" v-for="(item,key) in iconList" :key="key" :class="[key === currentIndex ? 'iconList' : '']" style="display: flex;padding: 5px 10px;" @mouseenter="mouseenterIcon(key)" @click="chooseIcon(key)">
            <icon :style="{ color: '#aaa','margin-right':'20px'}">
            <template #component>
              <svg  height="1.2em" fill="currentColor" viewBox="0 0 1024 1024">
              <path :d="item.d" p-id="item.p_id">
              </path>
              </svg>
              </template>
            </icon>
      ...
      <input type="text" v-model.trim="searchWord" class="searchWord" :placeholder="place" @keyup.enter="linkSearch" >
    </div>
    <div class="gohome" v-if="isShowTime"> {{ goHomeTime }}</div>
    <div class="textContent" :style="{'opacity': timeDisable ? '1' : '0'}">
      <!-- <p>{{ `「 ${textContent}」` }}</p> -->
      <p>{{  textContent }}</p>
      <div>
        <div style="display: flex;margin-top: 0px;font-size: 13px;color: rgba(255, 255, 255, 0.67);">
    ...
    <a-drawer
      ...
      style="color: rgb(255, 255, 255);background: rgba(190, 182, 182, 0.1);backdropFilter: blur(10px)"
      title="Basic Drawer"
      placement="right"
      width="500px"
      :closable="false"
    ...
    <a-tab-pane key="1" tab="设置">
      <div style="color: rgb(255, 255, 255);">
        <div>遮罩：</div>
        <a-slider v-model:value="maskNum" :min="0" :max="10" :tooltipOpen="false"/></div>
    </a-tab-pane>
  </a-tabs>
...
let now:any = new Date();  
  ...
  // 设置晚上7点的时间  
  let sevenPM:any = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 18, 40, 0);  
...
const userQueryEvent = () => {
  ...
  userQuery(data).then( (res: Object) => {
    console.log('res',res);
  })
}
const info = () => {
